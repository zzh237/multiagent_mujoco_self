...
def build_obs(env, k_dict, k_categories, global_dict, global_categories, vec_len=None, **kwargs):
    ...
    for k in sorted(list(k_dict.keys())):
        cats = k_categories[k] #['qpos','qvel']
        for _t in k_dict[k]:#[bfoot, bshin, bthigh]
            for c in cats:
                if c in _t.extra_obs:
                    ...
                    if c in ["qvel","qpos"]: # this is a "joint position/velocity" item
                        if state == None:
                            ...
                            items = state[c][getattr(_t, "{}_ids".format(c))]
                        obs_lst.extend(items if isinstance(items, list) else [items])
