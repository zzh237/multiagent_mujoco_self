...
class MujocoMulti(MultiAgentEnv):
    ...
    def step(self, actions):
        ...
        obs_n, reward_n, done_n, info_n = self.wrapped_env.step(flat_actions) #obs_n is the state 
        self.steps += 1
        obs_agents = self.get_obs_from_state(obs_n)#obs_agents are the obs with agents
        ...
        return obs_agents, reward_n, done_n, info
    ...
    def get_obs_from_state(self, state):
        ...
        for agent_id in range(self.n_agents):
            ob = build_obs(self.env,
                    ...
                    self.mujoco_globals,
                    self.global_categories,
                    vec_len=getattr(self, "obs_size", None), query_state = state)
            obs.append(ob)
        return obs 
